const $=id=>document.getElementById(id);let notes=[],faturistas=[],filtered=[],auditLogs=[],refreshing=false,filterProcessing=false,confirmCallback=null,pendingDeleteId=null,currentUser=null,tablePage=1,tablePageSize=100,tmeChart=null,tmacChart=null,tmacDetailChart=null;
...
async function loadAll(silent=false){if(refreshing)return;refreshing=true;try{const loadedNotes=getPermissions().notes?await fetchAllNotes(!silent):[];if(loadedNotes===null)return;notes=loadedNotes;if(getPermissions().users){faturistas=await api("/faturistas/?incluir_inativos=true")}else{faturistas=[];}renderBillingOptions();applyFilters({resetPage:false,showMessage:!silent});if(getPermissions().users)renderFaturistas();$("lastUpdate").textContent=`Atualizado ${new Date().toLocaleTimeString("pt-BR")}`;if(!silent)toast("Dados atualizados")}catch(e){toast(e.message,true)}finally{refreshing=false}}
