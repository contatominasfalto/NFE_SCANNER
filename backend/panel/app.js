...
function renderFaturistas(){const labels={notes:"Notas",reports:"Relatórios",tme:"TME",tmac:"TMAC",users:"Usuários",audit:"Rastreabilidade",swagger:"Swagger"},canManage=getPermissions().usersManage;$("billingList").innerHTML=faturistas.map(f=>{const system=f.nome.toLowerCase()==="adm"||f.nome==="BIPE",actions=!canManage?'<span>Somente visualização</span>':system?'<span>Conta protegida</span>':`<button class="secondary" onclick="editBilling(${f.id})">Editar</button><button class="danger" onclick="deleteBilling(${f.id})">Excluir</button><button class="secondary" onclick="toggleBilling(${f.id},${!f.ativo})">${f.ativo?"Desativar":"Reativar"}</button>`;return`<div class="billing-item"><div class="${f.ativo?"":"inactive"}"><strong>${esc(f.nome)}</strong><span class="billing-profile">${esc(PROFILE_NAMES[f.role]||f.role)}</span><small>${f.ativo?"Ativo":"Desativado"}</small><span class="billing-module-summary">${(f.modulos||[]).map(module=>labels[module]||module).join(" · ")||"Nenhum módulo"}</span></div><div>${actions}</div></div>`}).join("")}
function setBillingFormMode(mode,faturista=null){$("billingForm").hidden=!getPermissions().usersManage;if(!getPermissions().usersManage)return;const editing=mode==="edit",role=editing?faturista?.role:"user";$("billingId").value=editing?faturista?.id:"";$("billingName").value=editing?faturista?.nome:"";$("billingName").readOnly=editing;$("billingName").required=!editing;$("billingPassword").value="";$("billingPassword").required=!editing;$("billingRole").value=role;$("billingRole").disabled=editing&&faturista?.nome==="viewer_user";setBillingModules(editing?faturista?.modulos:PROFILE_DEFAULT_MODULES[role]);$("billingSubmit").textContent=editing?"Salvar alterações":"＋ Cadastrar"}
...
function closeDialog(id){const dialog=$(id);if(dialog?.open){dialog.close()}if(id==="reportsDialog")setModalMaximized("reportsModalSection",false);if(id==="tmeDialog")setModalMaximized("tmeModalSection",false);if(id==="tmacDialog")setModalMaximized("tmacModalSection",false)}
