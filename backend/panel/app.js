...
<td>${esc(n.transportador||"—")}</td><td>${esc(n.faturista||"BIPE")}</td><td>${esc(n.cnpj_fornecedor)}</td>
<td title="${esc(n.observacao)}">${esc(n.observacao||"—")}</td></tr>`}).join("");
$("emptyState").hidden=filtered.length>0;$("resultCount").textContent=`${filtered.length} registro(s)`;$("metricTotal").textContent=notes.length;$("metricWeight").textContent=new Intl.NumberFormat("pt-BR").format(notes.reduce((s,n)=>s+(n.quantidade||0),0))}
...
function exportTableExcel(){if(!filtered.length){toast("Nenhum registro para exportar.",true);return}const columns=[["id","ID"],["chave_acesso","Chave NF"],["data_cadastro","Data/hora do bip"],["data_emissao","Data emissão"],["nome_fornecedor","Fornecedor"],["numero_nf","NF"],["local","Local"],["produto","Produto"],["quantidade","Quantidade"],["valor_total","Valor total"],["transportador","Transportador"],["faturista","Usuário"],["cnpj_fornecedor","CNPJ"],["observacao","Observação"],["erro_salvamento","Com erro"],["erro_detalhe","Detalhe do erro"]];const rows=filtered.map(n=>`<Row>${columns.map(([key])=>{let value=n[key];if(["data_cadastro","data_emissao"].includes(key))value=date(value);if(key==="erro_salvamento")value=isErrorNote(n)?"Sim":"Não";const numeric=typeof value==="number"&&Number.isFinite(value);return excelCell(value,numeric?"Number":"String")}).join("")}</Row>`).join("");const header=`<Row>${columns.map(([,label])=>excelCell(label)).join("")}</Row>`;const workbook=`<?xml version="1.0" encoding="UTF-8"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="Notas fiscais"><Table>${header}${rows}</Table></Worksheet></Workbook>`;const blob=new Blob([workbook],{type:"application/vnd.ms-excel;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`notas_painel_${new Date().toISOString().slice(0,10)}.xls`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);toast("Excel da tabela baixado")}
