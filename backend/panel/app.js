...
function applyAccessRules(){
	...
	setVisible("openTphbReport",p.reports);
	setVisible("openTpheReport",p.reports);
	...
	setVisible("exportTphbPdf",p.reports&&p.canDownload);
	setVisible("exportTphePdf",p.reports&&p.canDownload);
...
async function openThroughputModal(prefix){if(!getPermissions().reports){toast("Acesso não autorizado ao relatório.",true);return}if(!$(prefix+"Date").value)$(prefix+"Date").value=localDateKey(new Date());setModalMaximized(prefix+"ModalSection",true);if(!$(prefix+"Dialog").open)$(prefix+"Dialog").showModal();await loadThroughputReport(prefix)}
