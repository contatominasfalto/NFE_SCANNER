...
async function loadAuditLogs(){const button=$("reloadAudit"),original=button?.textContent;if(button){button.disabled=true;button.textContent="Atualizando..."}try{const logs=await api("/auditoria/?limit=300");renderAuditLogs(logs)}catch(error){toast(error.message,true)}finally{if(button){button.disabled=false;button.textContent=original}}}
