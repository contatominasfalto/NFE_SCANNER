const $=id=>document.getElementById(id);let notes=[],faturistas=[],filtered=[],auditLogs=[],refreshing=false,filterProcessing=false,searchTimer=null,confirmCallback=null,pendingDeleteId=null,currentUser=null,tablePage=1,tablePageSize=100,tmeChart=null,tmacChart=null,tmacDetailChart=null;
...
function scheduleGlobalSearch(){clearTimeout(searchTimer);searchTimer=setTimeout(()=>processGlobalSearch(),420)}
async function processGlobalSearch(){if(filterProcessing)return;const query=$("searchInput").value.trim(),started=Date.now(),message=query?"Pesquisando em todo o banco de dados, sem limitar pelo período.":"Restaurando as notas do período de Bip selecionado.";setFilterProcessing(true,message);try{while(refreshing)await delay(80);await delay(40);await loadAll(true);const remaining=450-(Date.now()-started);if(remaining>0)await delay(remaining)}catch(error){toast(error.message||"Não foi possível realizar a pesquisa.",true)}finally{setFilterProcessing(false)}}
...
function closeDialog(id){const dialog=$(id);if(dialog?.open){dialog.close()}if(id==="reportsDialog")setModalMaximized("reportsModalSection",false);if(id==="tmeDialog")setModalMaximized("tmeModalSection",false);if(id==="tmacDialog")setModalMaximized("tmacModalSection",false);if(id==="faturistasDialog")setModalMaximized("usersModalSection",false)}
