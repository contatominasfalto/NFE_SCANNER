const $=id=>document.getElementById(id);let notes=[],faturistas=[],filtered=[],auditLogs=[],refreshing=false,confirmCallback=null,currentUser=null,tablePage=1,tablePageSize=100;
...
async function fixIssueTimezone(){
	confirm("Corrigir emissão", "Isso aplicará UTC-3 nas notas sem erro com emissão entre 00:00 e 02:59. Continuar?", async()=>{
		const button=$("fixIssueTimezoneButton"),original=button?.textContent;
		if(button){button.disabled=true;button.textContent="Corrigindo..."}
		try{
			const result=await api("/notas/corrigir-fuso-emissao/",{method:"POST"});
			await loadAll(true);
			toast(`${result.corrigidas} nota(s) corrigida(s)`);
