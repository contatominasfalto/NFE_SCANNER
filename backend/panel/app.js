const $=id=>document.getElementById(id);let notes=[],faturistas=[],filtered=[],auditLogs=[],refreshing=false,filterProcessing=false,searchTimer=null,confirmCallback=null,pendingDeleteId=null,currentUser=null,tablePage=1,tablePageSize=100,tmeChart=null,tmacChart=null,tmacDetailChart=null;const throughputCharts={};
const SHOW_USER_FILTER=false;
const PROFILE_DEFAULT_MODULES={admin:["notes","reports","tme","tmac","tphb","tphe","users","audit","swagger"],user:["notes","reports","tphb","tphe"],viewer:["notes","reports","tme","tmac","tphb","tphe"]};
...
const issueDate=v=>{if(!v)return"—";const m=String(v).match(/^(\d{4})-(\d{2})-(\d{2})(?:[T\s](\d{2}):(\d{2})(?::(\d{2}))?)?/);if(!m)return date(v);const time=m[4]?`, ${m[4]}:${m[5]}:${m[6]||"00"}`:"";return`${m[3]}/${m[2]}/${m[1]}${time}`};
...
function setFilterProcessing(active,message="Aguarde enquanto os dados do período selecionado são preparados."){const overlay=$("filterProcessingOverlay");if(!overlay)return;filterProcessing=active;overlay.hidden=!active;overlay.setAttribute("aria-hidden",String(!active));$("filterProcessingMessage").textContent=message;document.body.classList.toggle("filter-processing",active);document.querySelector("main")?.toggleAttribute("inert",active);document.querySelector(".sidebar")?.toggleAttribute("inert",active)}
async function processDateFilters(reloadFromServer){if(filterProcessing)return;const bipRange=getDateRange("bipStartDateFilter","bipEndDateFilter","Bip"),issueRange=getDateRange("issueStartDateFilter","issueEndDateFilter","Emissao");if(bipRange===false||issueRange===false)return;const started=Date.now(),message=reloadFromServer?"Consultando as notas e aplicando o período de Bip selecionado.":"Aplicando o período de Emissão sobre as notas carregadas.";setFilterProcessing(true,message);try{while(refreshing)await delay(80);await delay(40);if(reloadFromServer)await loadAll(true);else applyFilters();const remaining=450-(Date.now()-started);if(remaining>0)await delay(remaining)}catch(error){toast(error.message||"Não foi possível aplicar o filtro.",true)}finally{setFilterProcessing(false)}}
function scheduleGlobalSearch(wait=700){clearTimeout(searchTimer);searchTimer=setTimeout(()=>processGlobalSearch(),wait)}
async function processGlobalSearch(){if(filterProcessing){scheduleGlobalSearch(200);return}if(refreshing){scheduleGlobalSearch(150);return}const requestedQuery=$("searchInput").value.trim();await loadAll(true);if($("searchInput").value.trim()!==requestedQuery)scheduleGlobalSearch(0)}
...
function applyAccessRules(){
	...
	setVisible("downloadAll",p.download);
	setVisible("downloadTableExcel",p.download);
	setVisible("refreshErrorsButton",p.refreshErrors);
	setVisible("exportReportPdf",p.reports&&p.canDownload);
	setVisible("exportReportExcel",p.reports&&p.canDownload);
	setVisible("exportTmePdf",p.tme&&p.canDownload);
	setVisible("exportTmacPdf",p.tmac&&p.canDownload);
	setVisible("exportTphbPdf",p.tphb&&p.canDownload);
	setVisible("exportTphePdf",p.tphe&&p.canDownload);
	setVisible("exportAuditExcel",p.audit&&p.canDownload);
}
async function ensureAuthenticated(){try{currentUser=await api("/auth/me/");applyAccessRules();setDefaultBipRange();hideLogin();return true;}catch(err){currentUser=null;applyAccessRules();showLogin("");return false;}}
...
function buildActiveNotesQuery(skip,limit,showMessage=true){const query=$("searchInput").value.trim();if(query)return new URLSearchParams({skip:String(skip),limit:String(limit),busca:query});return buildBipQuery(skip,limit,showMessage)}
...
async function loadAll(silent=false){if(refreshing)return;refreshing=true;try{const loadedNotes=getPermissions().notes?await fetchAllNotes(!silent,buildActiveNotesQuery):[];if(loadedNotes===null)return;notes=loadedNotes;if(getPermissions().users){faturistas=await api("/faturistas/?incluir_inativos=true")}else{faturistas=[];}renderBillingOptions();applyFilters({resetPage:false,showMessage:!silent});if(getPermissions().users)renderFaturistas();$("lastUpdate").textContent=`Atualizado ${new Date().toLocaleTimeString("pt-BR")}`;if(!silent)toast("Dados atualizados")}catch(e){toast(e.message,true)}finally{refreshing=false}}
...
function applyFilters(options={resetPage:true,showMessage:true}){const q=$("searchInput").value.trim();if(q){filtered=sortNotesForTable([...notes]);if(options.resetPage)tablePage=1;render();return}const onlyErrors=$("errorOnlyFilter").checked,bipRange=getDateRange("bipStartDateFilter","bipEndDateFilter","Bip",options.showMessage),issueRange=getDateRange("issueStartDateFilter","issueEndDateFilter","Emissao",options.showMessage),local=$("localFilter").value,bill=$("billingFilter").value;if(bipRange===false||issueRange===false)return;filtered=sortNotesForTable(notes.filter(n=>(!onlyErrors||isErrorNote(n))&&inDateRange(n.data_cadastro,bipRange)&&inDateRange(n.data_emissao,issueRange)&&(!local||n.local===local)&&(!bill||n.faturista===bill)));if(options.resetPage)tablePage=1;render()}
...
function renderMobileNotes(rows){const container=$("mobileNotes");if(!container)return;if(!filtered.length){container.innerHTML='<div class="mobile-empty">Nenhuma nota encontrada com os filtros atuais.</div>';return}const canManageNotes=currentUser?.role==="admin",canDownloadXml=getPermissions().download;container.innerHTML=rows.map(n=>{const error=isErrorNote(n),editAction=canManageNotes?`<button type="button" title="Editar" onclick="editNote(${n.id})">Editar</button>`:"",xmlAction=canDownloadXml&&!error?`<button type="button" title="Gerar XML" onclick="downloadXML(${n.id})">XML</button>`:"",refreshAction=canManageNotes&&!error?`<button type="button" title="Reprocessar NF-e" onclick="refreshNote(${n.id})">Reprocessar</button>`:"",deleteAction=canManageNotes?`<button type="button" class="danger-link" title="Excluir" onclick="askDelete(${n.id})">Excluir</button>`:"";if(error){return `<article class="mobile-note-card save-error"><div class="mobile-note-head"><div><span>Nota com erro</span><strong>${esc(n.numero_nf||"ERRO")}</strong></div><span class="badge">${esc(n.local||"-")}</span></div><div class="mobile-note-key">${esc(n.chave_acesso)}</div><dl><div><dt>Bip</dt><dd>${date(n.data_cadastro)}</dd></div><div><dt>Usuario</dt><dd>${esc(displayFaturista(n))}</dd></div><div><dt>Motivo</dt><dd>${esc(n.erro_detalhe||"Falha ao salvar nota")}</dd></div></dl><div class="mobile-note-actions">${editAction}${deleteAction}</div></article>`}return `<article class="mobile-note-card"><div class="mobile-note-head"><div><span>NF</span><strong>${esc(n.numero_nf||"-")}</strong></div><span class="badge">${esc(n.local||"-")}</span></div><div class="mobile-note-product">${esc(n.produto||"-")}</div><dl><div><dt>Bip</dt><dd>${date(n.data_cadastro)}</dd></div><div><dt>Emissao</dt><dd>${issueDate(n.data_emissao)}</dd></div><div><dt>Fornecedor</dt><dd>${esc(n.nome_fornecedor||"-")}</dd></div><div><dt>Quantidade</dt><dd>${esc(n.quantidade||"-")}</dd></div><div><dt>Valor</dt><dd>${money(n.valor_total)}</dd></div><div><dt>Usuario</dt><dd>${esc(displayFaturista(n))}</dd></div></dl><div class="mobile-note-key">${esc(n.chave_acesso)}</div><div class="mobile-note-actions">${editAction}${xmlAction}${refreshAction}${deleteAction}</div></article>`}).join("")}
function render(){const body=$("notesTable"),page=getTablePage();body.innerHTML=page.rows.map(n=>{
	const canManageNotes = getPermissions().notesManage;
	const canDownloadXml = getPermissions().download;
	...
	const refreshAction = canManageNotes?`<button title="Reprocessar NF-e" onclick="refreshNote(${n.id})">↻</button>`:"";
...
<td><span class="badge">${esc(n.local||"—")}</span></td>${errorCell.repeat(4)}<td>${esc(displayFaturista(n))}</td>${errorCell.repeat(2)}</tr>`;
	...
	return `<tr>
<td class="sticky"><div class="row-actions">${editAction}${xmlAction}${refreshAction}${deleteAction}</div></td>
<td title="${esc(n.chave_acesso)}">${esc(n.chave_acesso)}</td><td>${date(n.data_cadastro)}</td><td>${issueDate(n.data_emissao)}</td><td>${esc(n.nome_fornecedor)}</td><td><strong>${esc(n.numero_nf)}</strong></td>
...
$("emptyState").hidden=filtered.length>0;renderMobileNotes(page.rows);renderPagination(page);updateSubtotalMetrics()}
function renderBillingOptions(){const active=faturistas.length?faturistas.filter(f=>f.ativo&&f.role!=="admin"):[...new Set(notes.map(n=>n.faturista).filter(Boolean))].sort((a,b)=>a.localeCompare(b)).map(nome=>({nome,ativo:true})),current=$("billingFilter").value;$("billingFilter").innerHTML='<option value="">Usuários</option>'+active.map(f=>`<option>${esc(f.nome)}</option>`).join("");$("billingFilter").value=current;$("faturista").innerHTML=active.map(f=>`<option>${esc(f.nome)}</option>`).join("")}
function billingModules(){return[...document.querySelectorAll('[name="billingModule"]:checked')].map(input=>input.value)}
function setBillingModules(modules){const admin=$("billingRole").value==="admin";document.querySelectorAll('[name="billingModule"]').forEach(input=>{input.checked=admin||(modules||[]).includes(input.value);input.disabled=admin})}
function renderFaturistas(){const labels={notes:"Notas Fiscais",reports:"Relatórios",tme:"TME",tmac:"TMAC",tphb:"TPHB",tphe:"TPHE",users:"Usuários",audit:"Rastreabilidade",swagger:"Swagger API"},canManage=getPermissions().usersManage;$("billingList").innerHTML=faturistas.map(f=>{const system=f.nome.toLowerCase()==="adm"||f.nome==="BIPE",actions=!canManage?'<span class="user-action-note">Somente visualização</span>':system?'<span class="user-action-note protected">Conta protegida</span>':`<button class="secondary" onclick="editBilling(${f.id})">Editar</button><button class="secondary" onclick="toggleBilling(${f.id},${!f.ativo})">${f.ativo?"Desativar":"Reativar"}</button><button class="danger" onclick="deleteBilling(${f.id})">Excluir</button>`,modules=(f.modulos||[]).map(module=>`<span class="user-module-chip">${esc(labels[module]||module)}</span>`).join("")||'<span class="user-module-empty">Nenhum módulo</span>';return`<article class="user-row ${f.ativo?"":"inactive"}"><div class="user-identity"><strong>${esc(f.nome)}</strong><small>${system?"Conta do sistema":"Conta de acesso"}</small></div><div><span class="billing-profile profile-${esc(f.role)}">${esc(PROFILE_NAMES[f.role]||f.role)}</span></div><div><span class="user-status ${f.ativo?"active":"disabled"}"><i></i>${f.ativo?"Ativo":"Desativado"}</span></div><div class="user-modules">${modules}</div><div class="user-actions">${actions}</div></article>`}).join("")}
function setBillingFormMode(mode,faturista=null){$("billingForm").hidden=!getPermissions().usersManage;if(!getPermissions().usersManage)return;const editing=mode==="edit",role=editing?faturista?.role:"user";$("billingId").value=editing?faturista?.id:"";$("billingName").value=editing?faturista?.nome:"";$("billingName").readOnly=editing;$("billingName").required=!editing;$("billingPassword").value="";$("billingPassword").required=!editing;$("billingRole").value=role;$("billingRole").disabled=editing&&faturista?.nome==="viewer_user";setBillingModules(editing?faturista?.modulos:PROFILE_DEFAULT_MODULES[role]);$("billingFormTitle").textContent=editing?`Editar ${faturista?.nome}`:"Novo usuário";$("billingFormHint").textContent=editing?"Ajuste o perfil, a senha ou os módulos permitidos para esta conta.":"Informe as credenciais, escolha um perfil e confirme os módulos permitidos.";$("billingSubmit").textContent=editing?"Salvar alterações":"＋ Cadastrar usuário"}
function setModalMaximized(targetId,maximized){const target=$(targetId);if(!target)return;target.classList.toggle("maximized",maximized);document.querySelectorAll(`[data-maximize="${targetId}"]`).forEach(btn=>{btn.setAttribute("aria-pressed",String(maximized));btn.setAttribute("aria-label",maximized?"Restaurar relatório":"Maximizar relatório");btn.title=maximized?"Restaurar relatório":"Maximizar relatório";btn.textContent=maximized?"❐":"□"})}
function closeDialog(id){const dialog=$(id);if(dialog?.open){dialog.close()}if(id==="reportsDialog")setModalMaximized("reportsModalSection",false);if(id==="tmeDialog")setModalMaximized("tmeModalSection",false);if(id==="tmacDialog")setModalMaximized("tmacModalSection",false);if(id==="tphbDialog")setModalMaximized("tphbModalSection",false);if(id==="tpheDialog")setModalMaximized("tpheModalSection",false);if(id==="faturistasDialog")setModalMaximized("usersModalSection",false)}
...
document.querySelectorAll("[data-maximize]").forEach(btn=>btn.addEventListener("click",e=>{e.preventDefault();const target=$(btn.dataset.maximize);if(target)setModalMaximized(btn.dataset.maximize,!target.classList.contains("maximized"))}));
...
function exportTableExcel(){if(!getPermissions().download){toast("Acesso nao autorizado.",true);return}if(!filtered.length){toast("Nenhum registro para exportar.",true);return}const columns=[["id","ID"],["chave_acesso","Chave NF"],["data_cadastro","Data/hora do bip"],["data_emissao","Data emissão"],["nome_fornecedor","Fornecedor"],["numero_nf","NF"],["local","Local"],["produto","Produto"],["quantidade","Quantidade"],["valor_total","Valor total"],["transportador","Transportador"],["faturista","Usuário"],["cnpj_fornecedor","CNPJ"],["observacao","Observação"],["erro_salvamento","Com erro"],["erro_detalhe","Detalhe do erro"]];const summary=`<Row>${excelCell("Total cadastrado")}${excelCell(filtered.length,"Number")}</Row><Row>${excelCell("Peso líquido")}${excelCell(subtotalQuantidade(filtered),"Number")}</Row><Row></Row>`;const rows=filtered.map(n=>`<Row>${columns.map(([key])=>{let value=key==="faturista"?displayFaturista(n):n[key];if(key==="data_cadastro")value=date(value);if(key==="data_emissao")value=issueDate(value);if(key==="erro_salvamento")value=isErrorNote(n)?"Sim":"Não";const numeric=typeof value==="number"&&Number.isFinite(value);return excelCell(value,numeric?"Number":"String")}).join("")}</Row>`).join("");const header=`<Row>${columns.map(([,label])=>excelCell(label)).join("")}</Row>`;const workbook=`<?xml version="1.0" encoding="UTF-8"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="Notas fiscais"><Table>${summary}${header}${rows}</Table></Worksheet></Workbook>`;const blob=new Blob([workbook],{type:"application/vnd.ms-excel;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`notas_painel_${new Date().toISOString().slice(0,10)}.xls`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);toast("Excel filtrado baixado")}
window.downloadXML=id=>downloadReport(`nota_id=${id}&formato=xml`,`nota_fiscal_${id}.xml`);
window.refreshNote=async id=>{try{await api(`/notas/${id}/refresh/`,{method:"POST"});toast("Nota reprocessada");await loadAll(true)}catch(e){toast(e.message,true)}};
window.askDelete=id=>{const nota=notes.find(n=>n.id===id);pendingDeleteId=id;$("deleteTitle").textContent=nota?.numero_nf?`Excluir NF ${nota.numero_nf}`:"Excluir nota fiscal";$("deleteReason").value="";$("deleteDialog").showModal();$("deleteReason").focus()}
window.toggleBilling=async(id,ativo)=>{const f=faturistas.find(x=>x.id===id);try{await api(`/faturistas/${id}/`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({nome:f.nome,ativo,role:f.role,modulos:f.modulos})});toast(ativo?"Usuário reativado":"Usuário desativado");loadAll(true)}catch(e){toast(e.message,true)}}
...
async function loadAuditLogs(){const button=$("reloadAudit"),original=button?.textContent;if(button){button.disabled=true;button.textContent="Atualizando..."}try{auditLogs=await api("/auditoria/?limit=300");$("auditEmpty").textContent="Nenhuma modificação registrada até o momento.";renderAuditLogs(auditLogs)}catch(error){auditLogs=[];renderAuditError(error.message);toast(error.message,true)}finally{if(button){button.disabled=false;button.textContent=original}}}
function exportAuditExcel(){if(!auditLogs.length){toast("Nenhum evento para exportar.",true);return}const columns=[["created_at","Data e hora"],["usuario","Usuario"],["acao","Acao"],["area","Onde fez"],["registro","Registro"],["descricao","O que fez"],["detalhes","Detalhes"]];const rows=auditLogs.map(log=>({created_at:auditDate(log.created_at),usuario:log.usuario,acao:log.acao,area:log.area,registro:`${log.entidade||"—"}${log.entidade_id?` #${log.entidade_id}`:""}`,descricao:log.descricao,detalhes:log.detalhes||"—"})).map(row=>`<Row>${columns.map(([key])=>excelCell(row[key])).join("")}</Row>`).join("");const header=`<Row>${columns.map(([,label])=>excelCell(label)).join("")}</Row>`;const summary=`<Row>${excelCell("Rastreabilidade")}${excelCell(`${auditLogs.length} evento(s) listado(s)`)}</Row><Row></Row>`;const workbook=`<?xml version="1.0" encoding="UTF-8"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="Rastreabilidade"><Table>${summary}${header}${rows}</Table></Worksheet></Workbook>`;const blob=new Blob([workbook],{type:"application/vnd.ms-excel;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`rastreabilidade_${new Date().toISOString().slice(0,10)}.xls`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);toast("Excel de rastreabilidade baixado")}
async function openAuditLog(){if(!getPermissions().audit)return;$("auditDialog").showModal();$("auditSummary").innerHTML="<strong>Carregando eventos...</strong><span>Histórico administrativo de alterações</span>";$("auditTableBody").innerHTML="";$("auditEmpty").hidden=true;await loadAuditLogs()}
...
$("deleteForm").addEventListener("submit",async e=>{e.preventDefault();const id=pendingDeleteId,motivo=$("deleteReason").value.trim();if(!id)return;if(motivo.length<5){toast("Informe o motivo da exclusao com pelo menos 5 caracteres.",true);$("deleteReason").focus();return}const button=$("deleteSubmit"),original=button.textContent;button.disabled=true;button.textContent="Excluindo...";try{await api(`/notas/${id}/excluir/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({observacao_exclusao:motivo})});$("deleteDialog").close();toast("Nota excluida");await loadAll(true)}catch(err){toast(err.message,true)}finally{button.disabled=false;button.textContent=original}});$("deleteDialog").addEventListener("close",()=>{pendingDeleteId=null});$("noteForm").addEventListener("submit",async e=>{e.preventDefault();const payload={};fields.forEach(k=>payload[k]=$(k).value||null);payload.valor_total=Number(payload.valor_total||0);payload.quantidade=payload.quantidade?Number(payload.quantidade):null;payload.caminho_arquivo_imagem=null;const id=$("noteId").value;try{await api(id?`/notas/${id}/`:"/notas/",{method:id?"PUT":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});$("noteDialog").close();toast(id?"Nota atualizada":"Nota cadastrada");loadAll(true)}catch(err){toast(err.message,true)}});
$("batchForm").addEventListener("submit",async e=>{e.preventDefault();await submitBatch()});
$("billingForm").addEventListener("submit",async e=>{e.preventDefault();const nome=$("billingName").value.trim(),senha=$("billingPassword").value.trim(),id=$("billingId").value,role=$("billingRole").value,modulos=billingModules();try{if(id){const faturista=faturistas.find(f=>f.id===Number(id)),payload={nome:faturista.nome,ativo:faturista?.ativo??true,role,modulos};if(senha)payload.senha=senha;await api(`/faturistas/${id}/`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});toast("Usuário atualizado")}else{await api("/faturistas/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({nome,senha,role,modulos})});toast("Usuário cadastrado")}$("billingName").value="";$("billingPassword").value="";$("billingId").value="";setBillingFormMode("create");$("faturistasDialog").close();loadAll(true)}catch(err){toast(err.message,true)}});
$("billingRole").addEventListener("change",()=>setBillingModules(PROFILE_DEFAULT_MODULES[$("billingRole").value]||[]));
function editBilling(id){const faturista=faturistas.find(f=>f.id===id);if(!faturista)return;setBillingFormMode("edit",faturista);setModalMaximized("usersModalSection",true);$("faturistasDialog").showModal();$("billingPassword").focus();}
...
$("logoutButton").onclick=async()=>{try{await api("/auth/logout/",{method:"POST"});currentUser=null;applyAccessRules();showLogin("");}catch(err){toast(err.message,true)}};
$("openBatchScan").onclick=()=>{if(!getPermissions().batch){toast("Acesso nao autorizado.",true);return}$("batchForm").reset();$("batchDialog").showModal()};$("openFaturistas").addEventListener("click",async e=>{e.preventDefault();if(!getPermissions().users){toast("Acesso nao autorizado.",true);return}await loadAll(true);setBillingFormMode("create");setModalMaximized("usersModalSection",true);const faturistasDialog=$("faturistasDialog"); faturistasDialog.showModal ? faturistasDialog.showModal() : faturistasDialog.show();});$("openAudit").onclick=openAuditLog;$("reloadAudit").onclick=loadAuditLogs;$("exportAuditExcel").onclick=exportAuditExcel;$("refreshErrorsButton").onclick=refreshErrorNotes;$("refreshButton").onclick=()=>loadAll();$("searchInput").oninput=scheduleGlobalSearch;$("errorOnlyFilter").onchange=applyFilters;$("bipStartDateFilter").onchange=()=>processDateFilters(true);$("bipEndDateFilter").onchange=()=>processDateFilters(true);$("issueStartDateFilter").onchange=()=>processDateFilters(false);$("issueEndDateFilter").onchange=()=>processDateFilters(false);$("localFilter").onchange=applyFilters;$("billingFilter").onchange=applyFilters;
...
async function renderMaterialReport(){
	...
	const result=materialResultFromNotes(start,end,material);
...
const canAccessTme=()=>!!getPermissions().tme;
...
function renderTmacReport(result){
	$("tmacAverage").textContent=tmeMinutes(result.media_minutos);$("tmacMedian").textContent=tmeMinutes(result.mediana_minutos);$("tmacP90").textContent=tmeMinutes(result.p90_minutos);$("tmacLargest").textContent=tmeMinutes(result.maior_tempo_minutos);$("tmacNotes").textContent=new Intl.NumberFormat("pt-BR").format(result.total_notas);$("tmacIssues").textContent=`${result.total_inconsistencias} inconsistência(s) desconsiderada(s)`;
	$("tmacContext").textContent=result.total_notas?`${result.total_notas} nota(s) recebida(s) analisadas pelo intervalo entre emissão e bipe.`:"Não há notas válidas recebidas no período selecionado.";
	const ranking=result.maiores_tempos||[];$("tmacRankingBody").innerHTML=ranking.map((item,index)=>`<tr><td><span class="tme-rank">${index+1}</span></td><td>NF ${esc(item.numero_nf||"—")}</td><td title="${esc(item.chave_acesso||"")}">${esc(item.chave_acesso||"—")}</td><td>${tmeDateTime(item.emissao)}</td><td>${tmeDateTime(item.bipe)}</td><td><strong>${tmeMinutes(item.minutos)}</strong></td></tr>`).join("");$("tmacRankingEmpty").hidden=ranking.length>0;
	const allNotes=result.notas||[];$("tmacAllCount").textContent=`${new Intl.NumberFormat("pt-BR").format(allNotes.length)} notas, ordenadas do maior para o menor tempo.`;$("tmacAllBody").innerHTML=allNotes.map((item,index)=>`<tr><td>${index+1}</td><td>NF ${esc(item.numero_nf||"—")}</td><td title="${esc(item.chave_acesso||"")}">${esc(item.chave_acesso||"—")}</td><td>${tmeDateTime(item.emissao)}</td><td>${tmeDateTime(item.bipe)}</td><td><strong>${tmeMinutes(item.minutos)}</strong></td></tr>`).join("");$("tmacAllEmpty").hidden=allNotes.length>0;
	const days=result.dias||[];$("tmacChartEmpty").hidden=days.length>0;$("tmacChart").hidden=days.length===0;if(tmacChart){tmacChart.destroy();tmacChart=null}if(!days.length){if(tmacDetailChart){tmacDetailChart.destroy();tmacDetailChart=null}$("tmacDetailChart").hidden=true;$("tmacDetailChartEmpty").hidden=false;$("tmacDetailCaption").textContent="0 pontos — nenhuma nota fiscal válida.";return}
	tmacChart=new Chart($("tmacChart").getContext("2d"),{data:{labels:days.map(item=>item.data.split("-").reverse().slice(0,2).join("/")),datasets:[{type:"bar",label:"Notas recebidas",data:days.map(item=>item.quantidade_notas),yAxisID:"volume",backgroundColor:"rgba(104,151,186,.32)",borderColor:"#6897ba",borderWidth:1,borderRadius:4},{type:"line",label:"Média",data:days.map(item=>item.media_minutos),yAxisID:"tempo",borderColor:"#f29129",backgroundColor:"#f29129",borderWidth:2.5,tension:.25,pointRadius:3},{type:"line",label:"Mediana",data:days.map(item=>item.mediana_minutos),yAxisID:"tempo",borderColor:"#29292e",backgroundColor:"#29292e",borderWidth:2,tension:.25,pointRadius:2},{type:"line",label:"P90",data:days.map(item=>item.p90_minutos),yAxisID:"tempo",borderColor:"#c73b36",backgroundColor:"#c73b36",borderWidth:2,tension:.25,pointRadius:3}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:"index",intersect:false},plugins:{legend:{position:"bottom"},tooltip:{callbacks:{label:item=>item.dataset.yAxisID==="volume"?` ${item.dataset.label}: ${item.raw}`:` ${item.dataset.label}: ${tmeMinutes(item.raw)}`}}},scales:{x:{grid:{display:false},title:{display:true,text:"Dia do recebimento"}},tempo:{beginAtZero:true,position:"left",title:{display:true,text:"Tempo emissão até bipe"},ticks:{callback:value=>tmeMinutes(value)}},volume:{beginAtZero:true,position:"right",grid:{drawOnChartArea:false},title:{display:true,text:"Quantidade de notas"},ticks:{precision:0}}}}});
	const chronological=[...allNotes].sort((a,b)=>new Date(a.bipe)-new Date(b.bipe)||Number(a.id)-Number(b.id));$("tmacDetailCaption").textContent=`${new Intl.NumberFormat("pt-BR").format(chronological.length)} pontos — um para cada nota fiscal.`;$("tmacDetailChartEmpty").hidden=chronological.length>0;$("tmacDetailChart").hidden=chronological.length===0;if(tmacDetailChart){tmacDetailChart.destroy();tmacDetailChart=null}if(chronological.length){tmacDetailChart=new Chart($("tmacDetailChart").getContext("2d"),{type:"line",data:{labels:chronological.map(item=>item.numero_nf||item.chave_acesso||item.id),datasets:[{label:"Tempo até o recebimento",data:chronological.map(item=>item.minutos),borderColor:"rgba(242,145,41,.35)",backgroundColor:chronological.map(item=>item.minutos>=result.p90_minutos?"#c73b36":"#f29129"),pointRadius:chronological.length>800?1.5:chronological.length>300?2.5:3.5,pointHoverRadius:6,borderWidth:1,showLine:true,tension:0}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:"nearest",intersect:true},plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>{const note=chronological[items[0].dataIndex];return`NF ${note.numero_nf||"—"}`},label:item=>{const note=chronological[item.dataIndex];return[`Tempo: ${tmeMinutes(note.minutos)}`,`Emissão: ${tmeDateTime(note.emissao)}`,`Bipe: ${tmeDateTime(note.bipe)}`,`Chave: ${note.chave_acesso||"—"}`]}}}},scales:{x:{ticks:{autoSkip:true,maxTicksLimit:18,maxRotation:0},grid:{display:false},title:{display:true,text:"Notas em ordem cronológica de recebimento"}},y:{beginAtZero:true,title:{display:true,text:"Tempo entre emissão e bipe"},ticks:{callback:value=>tmeMinutes(value)}}}}})}
}
async function loadTmacReport(){const start=$("tmacStartDate").value,end=$("tmacEndDate").value;if(!start||!end){toast("Informe a data inicial e final.",true);return}if(start>end){toast("A data inicial deve ser anterior ou igual à data final.",true);return}const button=$("applyTmacFilter"),original=button.textContent;button.disabled=true;button.textContent="Analisando...";try{const params=new URLSearchParams({data_inicio:`${start}T00:00:00`,data_fim:`${end}T23:59:59`});renderTmacReport(await api(`/relatorios/tmac-recebimento/?${params}`))}catch(error){toast(error.message,true)}finally{button.disabled=false;button.textContent=original}}
async function exportTmacPdf(){const start=$("tmacStartDate").value,end=$("tmacEndDate").value;if(!start||!end){toast("Informe a data inicial e final.",true);return}if(start>end){toast("A data inicial deve ser anterior ou igual à data final.",true);return}const button=$("exportTmacPdf"),original=button.textContent;button.disabled=true;button.textContent="Gerando...";try{const params=new URLSearchParams({data_inicio:`${start}T00:00:00`,data_fim:`${end}T23:59:59`}),response=await fetch(`/relatorios/tmac-recebimento/exportar/?${params}`,{credentials:"include"});if(!response.ok){let message=`Erro ${response.status}`;try{message=(await response.json()).detail||message}catch{}throw new Error(message)}const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`relatorio_tmac_recebimento_${start}_${end}.pdf`;document.body.appendChild(link);link.click();link.remove();URL.revokeObjectURL(url);toast("Relatório TMAC em PDF baixado")}catch(error){toast(error.message,true)}finally{button.disabled=false;button.textContent=original}}
async function openTmacModal(){if(!getPermissions().tmac){toast("Acesso não autorizado ao Relatório TMAC.",true);return}const range=defaultTmeRange();if(!$("tmacStartDate").value)$("tmacStartDate").value=range.start;if(!$("tmacEndDate").value)$("tmacEndDate").value=range.end;setModalMaximized("tmacModalSection",true);if(!$("tmacDialog").open)$("tmacDialog").showModal();await loadTmacReport()}
$("tmacFilterForm")?.addEventListener("submit",async event=>{event.preventDefault();await loadTmacReport()});$("exportTmacPdf")?.addEventListener("click",exportTmacPdf);$("openTmacReport")?.addEventListener("click",async event=>{event.preventDefault();if(!await ensureAuthenticated())return;await openTmacModal()});

const throughputTon=value=>`${new Intl.NumberFormat("pt-BR",{minimumFractionDigits:0,maximumFractionDigits:3}).format(Number(value)||0)} TON`;
const throughputMonths=["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];
function replaceThroughputChart(key,canvasId,config){throughputCharts[key]?.destroy();throughputCharts[key]=new Chart($(canvasId).getContext("2d"),config)}
function throughputBarOptions(xTitle,average=0){return{responsive:true,maintainAspectRatio:false,interaction:{mode:"index",intersect:false},plugins:{legend:{position:"bottom"},tooltip:{callbacks:{label:item=>` ${item.dataset.label}: ${throughputTon(item.raw)}`}},throughputAverageLine:{average}},scales:{x:{grid:{display:false},title:{display:true,text:xTitle}},y:{beginAtZero:true,title:{display:true,text:"Toneladas"},ticks:{callback:value=>new Intl.NumberFormat("pt-BR").format(value)}}}}}
const throughputAverageLine={id:"throughputAverageLine",afterDraw(chart,_args,options){const average=Number(options.average||0);if(!average||!chart.scales.y)return;const{ctx,chartArea:{left,right},scales:{y}}=chart,yPos=y.getPixelForValue(average);ctx.save();ctx.strokeStyle="#c73b36";ctx.setLineDash([6,4]);ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(left,yPos);ctx.lineTo(right,yPos);ctx.stroke();ctx.fillStyle="#c73b36";ctx.font="bold 11px Arial";ctx.fillText(`Média: ${throughputTon(average)}`,left+6,yPos-6);ctx.restore()}};
function renderThroughputReport(prefix,result){const title=prefix==="tphb"?"bipada":"emitida";$(prefix+"Total").textContent=throughputTon(result.total_toneladas_dia);$(prefix+"Average").textContent=throughputTon(result.media_toneladas_hora);$(prefix+"Peak").textContent=throughputTon(result.pico_toneladas_hora);$(prefix+"PeakHour").textContent=result.hora_pico?`pico às ${result.hora_pico}`:"sem movimento";$(prefix+"Notes").textContent=new Intl.NumberFormat("pt-BR").format(result.total_notas_dia);$(prefix+"Window").textContent=`${result.horas_analisadas}h`;$(prefix+"Events").textContent=result.primeiro_evento?`${tmeDateTime(result.primeiro_evento)} até ${tmeDateTime(result.ultimo_evento)}`:"sem eventos";$(prefix+"Context").textContent=result.total_notas_dia?`${result.total_notas_dia} nota(s), total de ${throughputTon(result.total_toneladas_dia)} ${title} em ${result.data.split("-").reverse().join("/")}.`:`Nenhuma tonelada ${title} encontrada na data escolhida.`;$(prefix+"MonthCaption").textContent=`Média de ${throughputTon(result.media_diaria_mes)} em ${result.dias_com_movimento_mes} dia(s) com movimento.`;
	const hourly=result.horas||[],month=result.dias_mes||[],year=result.meses_ano||[];$(prefix+"HourlyEmpty").hidden=hourly.length>0;$(prefix+"HourlyChart").hidden=!hourly.length;if(throughputCharts[prefix+"Hourly"]){throughputCharts[prefix+"Hourly"].destroy();delete throughputCharts[prefix+"Hourly"]}if(hourly.length)replaceThroughputChart(prefix+"Hourly",prefix+"HourlyChart",{type:"bar",data:{labels:hourly.map(item=>item.rotulo),datasets:[{label:"Toneladas por hora",data:hourly.map(item=>item.toneladas),backgroundColor:"rgba(242,145,41,.72)",borderColor:"#e68a22",borderWidth:1,borderRadius:4}]},options:throughputBarOptions("Hora"),plugins:[throughputAverageLine]});
	const monthHasData=month.some(item=>item.toneladas>0);$(prefix+"MonthEmpty").hidden=monthHasData;$(prefix+"MonthChart").hidden=!monthHasData;if(throughputCharts[prefix+"Month"]){throughputCharts[prefix+"Month"].destroy();delete throughputCharts[prefix+"Month"]}if(monthHasData)replaceThroughputChart(prefix+"Month",prefix+"MonthChart",{type:"bar",data:{labels:month.map(item=>String(item.dia).padStart(2,"0")),datasets:[{label:"Total diário",data:month.map(item=>item.toneladas),backgroundColor:"rgba(104,151,186,.55)",borderColor:"#6897ba",borderWidth:1,borderRadius:3}]},options:throughputBarOptions("Dia do mês",result.media_diaria_mes),plugins:[throughputAverageLine]});
	const yearHasData=year.some(item=>item.media_toneladas_dia>0);$(prefix+"YearEmpty").hidden=yearHasData;$(prefix+"YearChart").hidden=!yearHasData;if(throughputCharts[prefix+"Year"]){throughputCharts[prefix+"Year"].destroy();delete throughputCharts[prefix+"Year"]}if(yearHasData)replaceThroughputChart(prefix+"Year",prefix+"YearChart",{type:"bar",data:{labels:year.map(item=>throughputMonths[item.mes-1]),datasets:[{label:"Média diária do mês",data:year.map(item=>item.media_toneladas_dia),backgroundColor:"rgba(41,41,46,.72)",borderColor:"#29292e",borderWidth:1,borderRadius:4}]},options:throughputBarOptions("Mês"),plugins:[throughputAverageLine]})}
async function loadThroughputReport(prefix){const selected=$(prefix+"Date").value;if(!selected){toast("Escolha a data do relatório.",true);return}const button=$("apply"+prefix[0].toUpperCase()+prefix.slice(1)+"Filter"),original=button.textContent;button.disabled=true;button.textContent="Calculando...";try{renderThroughputReport(prefix,await api(`/relatorios/${prefix}/?data=${encodeURIComponent(selected)}`))}catch(error){toast(error.message,true)}finally{button.disabled=false;button.textContent=original}}
async function exportThroughputPdf(prefix){const selected=$(prefix+"Date").value;if(!selected){toast("Escolha a data do relatório.",true);return}const button=$("export"+prefix[0].toUpperCase()+prefix.slice(1)+"Pdf"),original=button.textContent;button.disabled=true;button.textContent="Gerando...";try{const response=await fetch(`/relatorios/${prefix}/exportar/?data=${encodeURIComponent(selected)}`,{credentials:"include"});if(!response.ok){let message=`Erro ${response.status}`;try{message=(await response.json()).detail||message}catch{}throw new Error(message)}const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`relatorio_${prefix}_${selected}.pdf`;document.body.appendChild(link);link.click();link.remove();URL.revokeObjectURL(url);toast(`Relatório ${prefix.toUpperCase()} em PDF baixado`)}catch(error){toast(error.message,true)}finally{button.disabled=false;button.textContent=original}}
async function openThroughputModal(prefix){if(!getPermissions()[prefix]){toast("Acesso não autorizado ao relatório.",true);return}if(!$(prefix+"Date").value)$(prefix+"Date").value=localDateKey(new Date());setModalMaximized(prefix+"ModalSection",true);if(!$(prefix+"Dialog").open)$(prefix+"Dialog").showModal();await loadThroughputReport(prefix)}
for(const prefix of ["tphb","tphe"]){$(prefix+"FilterForm")?.addEventListener("submit",async event=>{event.preventDefault();await loadThroughputReport(prefix)});$("export"+prefix[0].toUpperCase()+prefix.slice(1)+"Pdf")?.addEventListener("click",()=>exportThroughputPdf(prefix));$("open"+prefix[0].toUpperCase()+prefix.slice(1)+"Report")?.addEventListener("click",async event=>{event.preventDefault();if(!await ensureAuthenticated())return;await openThroughputModal(prefix)})}
